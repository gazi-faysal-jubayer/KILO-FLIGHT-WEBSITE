...
// Team Kilo Flight Motorsport Theme Palette (Cyan Telemetry, Racing Crimson, Jute Gold, High-Tech Electric Blue)
const MOTORSPORT_COLORS = [
  { r: 0.0, g: 0.9, b: 1.0 },    // Cyan (#00E5FF)
  { r: 1.0, g: 0.16, b: 0.16 },  // Crimson Red (#FF2A2A)
  { r: 0.97, g: 0.8, b: 0.18 },  // Jute Gold (#F8CB2E)
  { r: 0.04, g: 0.52, b: 1.0 },  // Electric Blue (#0A84FF)
  { r: 0.83, g: 0.64, b: 0.35 }, // Jute Bio-composite (#D4A359)
];
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Explicitly set dimensions before WebGL initialization
    canvas.width = scaleByPixelRatio(window.innerWidth || 1920);
    canvas.height = scaleByPixelRatio(window.innerHeight || 1080);

    function getNextColor() {
      const c1 = MOTORSPORT_COLORS[colorIndex % MOTORSPORT_COLORS.length];
      const c2 = MOTORSPORT_COLORS[(colorIndex + 1) % MOTORSPORT_COLORS.length];
      const t = colorTimer;
      return {
        r: c1.r * (1 - t) + c2.r * t,
        g: c1.g * (1 - t) + c2.g * t,
    ...
    const pointers: Array<ReturnType<typeof pointerPrototype>> = [pointerPrototype()];

    const contextObj = getWebGLContext(canvas);
    if (!contextObj) return;

    const gl = contextObj.gl;
    const ext = contextObj.ext;
    ...
    function getWebGLContext(c: HTMLCanvasElement) {
      ...
      let context = (c.getContext('webgl2', params) ||
        c.getContext('webgl', params) ||
        c.getContext('experimental-webgl', params)) as (WebGLRenderingContext | WebGL2RenderingContext | null);

      if (!context) return null;

      const isWebGL2 = Boolean('WebGL2RenderingContext' in window && context instanceof WebGL2RenderingContext);
      ...
      if (isWebGL2) {
        ...
        const gl1 = context as WebGLRenderingContext;
    ...
    class Material {
      ...
      setKeywords(keywords: string[]) {
        let hash = 0;
        for (let i = 0; i < keywords.length; i++) hash += 1 << i;

        let program = this.programs[hash];
        if (program == null) {
          let fragmentShader = compileShader(gl.FRAGMENT_SHADER, this.fragmentShaderSourceWithKeywords(keywords));
      ...
      fragmentShaderSourceWithKeywords(keywords: string[]) {
        let prefix = '';
        keywords.forEach((k) => (prefix += `#define ${k}\n`));
        return prefix + this.fragmentShaderSource;
      }
    ...
    displayMaterial.fragmentShaderSource = displayShaderSource;
    ...
    function resizeFBO(target: any, w: number, h: number, internalFormat: any, format: any, type: any, param: any) {
      ...
      clearShader.bind();
      gl.uniform1i(clearShader.uniforms.uTexture, target.attach(0));
      gl.uniform1f(clearShader.uniforms.value, 1);
      blit(newFBO);
      return newFBO;
    }
    ...
    function getResolution(resolution: number) {
      let aspectRatio = (gl.drawingBufferWidth || 1920) / (gl.drawingBufferHeight || 1080);
      if (aspectRatio < 1) aspectRatio = 1.0 / aspectRatio;

      const min = Math.max(16, Math.round(resolution));
      const max = Math.max(16, Math.round(resolution * aspectRatio));
    ...
    function step(dt: number) {
      ...
      if (!ext.supportLinearFiltering) gl.uniform2f(advectionShader.uniforms.dyeTexelSize, velocity.texelSizeX, velocity.texelSizeY);
      ...
      if (!ext.supportLinearFiltering) gl.uniform2f(advectionShader.uniforms.dyeTexelSize, dye.texelSizeX, dye.texelSizeY);
    ...
    function updateColors(dt: number) {
      colorTimer += dt * (config.COLOR_UPDATE_SPEED / 10);
      if (colorTimer >= 1) {
        colorTimer = 0;
        colorIndex++;
      }
    ...
    function updatePointerDownData(p: any, id: number, posX: number, posY: number) {
      p.id = id;
      p.down = true;
      p.moved = false;
    ...
    const handleMouseDown = (e: MouseEvent) => {
      ...
      // Create instant splat on click
      const color = getNextColor();
      color.r *= 5.0;
      color.g *= 5.0;
      color.b *= 5.0;
      splat(pointer.texcoordX, pointer.texcoordY, (Math.random() - 0.5) * 500, (Math.random() - 0.5) * 500, color);
    };
    ...
    // Initial burst to show active state
    multipleSplats(5);
